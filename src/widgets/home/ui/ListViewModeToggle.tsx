...
const ListViewModeToggle = () => {
    const { isMobile, listViewMode, setListViewMode } = useLayoutStore();

    return (
        <motion.article
            className={`fixed z-100 flex gap-[0.4rem] p-[0.4rem] rounded-[1.4rem] left-[50%] -translate-x-1/2 bg-[#ffffffd1] backdrop-blur-md shadow-[var(--shadow-normal)] ${
                isMobile ? "bottom-[calc(1.6rem+5.6rem)]" : "bottom-[calc(1.6rem*2)]"
            }`}
