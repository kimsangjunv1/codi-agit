...
const HomeViewModeNav = ({ variant }: HomeViewModeNavProps) => {
    ...
    const buttonTextClass = isFloating ? "font-extrabold text-[3.2rem]" : "font-extrabold text-[3.2rem]";

    return (
        <section className="w-full">
            <section className="max-w-[var(--size-pc)] px-[2.0rem] mx-auto flex items-center justify-start gap-[2.4rem] pt-[var(--header-height)]">
                <AnimatePresence mode="popLayout">
                    <UI.Button
                        key="search"
                        onClick={() => setIsSearchOpen(true)}
                        className={`${activeTextClass} ${buttonTextClass}`}
                    >
                    ...
                    {mainViewMode === 2 && (
                        <motion.section
                            ...
                            exit={{ opacity: 0 }}
                            transition={springTransition}
                            className={`flex gap-[1.6rem] justify-center ${isFloating ? "overflow-x-auto max-w-full scrollbar-hide" : ""}`}
                        >
                            <motion.section
                                key="category-all"
                                initial={{ opacity: 0, transform: "scale(0.8)" }}
